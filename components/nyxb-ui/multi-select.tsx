...
interface MultiSelectTriggerProps extends React.ComponentPropsWithoutRef<typeof Primitive.div> {}
    ...
    return (
      <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
        <div
          ...
          {...props}
          className={cn(
            'border-input ring-offset-background focus:ring-ring flex size-full min-h-10 items-center justify-between whitespace-nowrap rounded-md border bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-1 [&>span]:line-clamp-1',
...
interface MultiSelectValueProps extends React.ComponentPropsWithoutRef<typeof Primitive.div> {
  placeholder?: string
  maxDisplay?: number
...
const MultiSelectContent = React.forwardRef<React.ElementRef<typeof PopoverPrimitive.Content>, MultiSelectContentProps>(
