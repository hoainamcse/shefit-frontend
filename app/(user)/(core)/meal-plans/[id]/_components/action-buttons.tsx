...
export function ActionButtons({ mealPlanID }: ActionButtonsProps) {
  ...
  const saveQuery = useQuery({
    queryKey: ['user-saved-resources', session?.userId, 'meal-plan', mealPlanID],
  ...
  const saveMutation = useMutation({
    mutationFn: () => addUserSavedResource(session!.userId, 'meal_plan', mealPlanID),
    onSuccess: () => {
      saveQuery.refetch()
      toast.success('Đã thêm thực đơn vào danh sách!')
    },
    onError: (error) => {
      toast.error(error.message || 'Có lỗi xảy ra khi lưu thực đơn!')
    },
  })
