...
export async function createWeek(course_id: string, week: any): Promise<ApiResponse<Week>> {
...
export async function updateWeek(course_id: string, week_id: string, week: any): Promise<ApiResponse<Week>> {
...
export async function deleteWeek(course_id: string, week_id: string): Promise<ApiResponse<any>> {
    const response = await fetchData(`/v1/courses/${course_id}/video-classes/weeks/${week_id}`, {
        method: "DELETE",
    })
    revalidateTag(`weeks:${course_id}`)
